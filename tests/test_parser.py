...
from codex_runtime_hud import IncrementalReaderPool, IncrementalRolloutReader, RateLimitWindow, RateLimits, RolloutCandidate, RolloutParser, RootThreadSelector, SessionSelection, attach_rate_limits, parse_rollout, prefer_rate_limits, resolve_language
...
class HudTests(unittest.TestCase):
    ...
    def test_account_quotas_create_metrics_without_a_local_session(self):
        limits = RateLimits(
            five_hour=RateLimitWindow(used_percent=25, window_minutes=300),
            weekly=RateLimitWindow(used_percent=40, window_minutes=10080),
        )
        metrics = attach_rate_limits(None, limits, "turn")
        self.assertIsNotNone(metrics)
        self.assertEqual(metrics.scope, "turn")
        self.assertEqual(metrics.five_hour_limit.remaining_percent, 75.0)
        self.assertEqual(metrics.weekly_limit.remaining_percent, 60.0)
