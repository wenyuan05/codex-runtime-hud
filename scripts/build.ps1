...
py -3 (Join-Path $PSScriptRoot 'make_icon.py') --output (Join-Path $iconDir 'codex-token-overlay.ico')
...
py -3 -m PyInstaller --noconfirm --clean --distpath $distDir --workpath $workDir (Join-Path $PSScriptRoot '..\packaging\CodexTokenOverlay.spec')
if ($LASTEXITCODE -ne 0) { throw "PyInstaller failed with exit code $LASTEXITCODE." }

$exe = Join-Path $repoRoot 'dist\CodexTokenOverlay.exe'
if (-not (Test-Path -LiteralPath $exe)) { throw 'PyInstaller did not produce the EXE.' }
$hash = (Get-FileHash -LiteralPath $exe -Algorithm SHA256).Hash.ToLowerInvariant()
Set-Content -LiteralPath (Join-Path $repoRoot 'dist\SHA256SUMS.txt') -Encoding utf8 -Value "$hash  CodexTokenOverlay.exe"
